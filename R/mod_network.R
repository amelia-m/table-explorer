# ============================================================
# mod_network.R - Network overview (force-directed visNetwork view)
# ============================================================

#' Network overview module UI
#' @noRd
...
#' Network overview module server
#'
#' @param id Module id
...
mod_network_server <- function(
  ...
  moduleServer(id, function(input, output, session) {
      ...
      tryCatch(
        {
          ...
          if (layout_mode == "circular" && nrow(net$nodes) > 0) {
    ...
    output$node_panel_ui <- renderUI({
      ...
      # Node ids are table names
      req(node_id %in% names(tbls))
      t <- node_id
      df <- tbls[[t]]
      pk_v <- pks[[t]]
      cpk_groups <- cpks[[t]]
